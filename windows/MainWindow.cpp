...
void MainWindow::setupTopBar()
{
    ...
    m_topBarOpacityEffect = new QGraphicsOpacityEffect(m_topBar);
    m_topBar->setGraphicsEffect(m_topBarOpacityEffect);
    m_topBarOpacityEffect->setOpacity(1.0);
...
void MainWindow::setupBottomBar()
{
    ...
    m_bottomBarOpacityEffect = new QGraphicsOpacityEffect(m_bottomBar);
    m_bottomBar->setGraphicsEffect(m_bottomBarOpacityEffect);
    m_bottomBarOpacityEffect->setOpacity(1.0);
...
void MainWindow::setupBrightnessBar()
{
    ...
    m_brightnessBarOpacityEffect = new QGraphicsOpacityEffect(m_brightnessBar);
    m_brightnessBar->setGraphicsEffect(m_brightnessBarOpacityEffect);
    m_brightnessBarOpacityEffect->setOpacity(0.0);
...
void MainWindow::setupVolumeBar()
{
    ...
    m_volumeBarOpacityEffect = new QGraphicsOpacityEffect(m_volumeBar);
    m_volumeBar->setGraphicsEffect(m_volumeBarOpacityEffect);
    m_volumeBarOpacityEffect->setOpacity(0.0);
...
bool MainWindow::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
{
    ...
    if (msg->message == WM_WINDOWPOSCHANGING) {
        ...
        wp->flags |= SWP_NOCOPYBITS; // Prevent layout thrashing / white flash resizing glitches
    }
#endif
...
{
    return QFile::exists(":/shaders/Anime4K_Clamp_Highlights.glsl");
}
...
{
    m_lastPositionTime = QDateTime::currentMSecsSinceEpoch();
    if (m_isSeeking || m_seekSlider->isSliderDown()) return;
    if (m_duration > 0) {
...
void MainWindow::updateHoverBars()
{
    ...
    if (!draggingLeft && !draggingRight) {
        if (!m_mpvWidget->rect().contains(localPos) ||
            ...
            animateWidgetOpacity(m_brightnessBar, m_brightnessBarOpacityEffect, 0.0, 250);
            animateWidgetOpacity(m_volumeBar, m_volumeBarOpacityEffect, 0.0, 250);
            return;
        }
    }
    ...
    animateWidgetOpacity(m_brightnessBar, m_brightnessBarOpacityEffect, inLeftZone ? 1.0 : 0.0, 250);
    animateWidgetOpacity(m_volumeBar, m_volumeBarOpacityEffect, inRightZone ? 1.0 : 0.0, 250);
}
...
void MainWindow::updateResolutionBadge()
{
    ...
    // Determine viewport size using device pixel ratio to match physical screen pixels
    double dpr = m_mpvWidget->devicePixelRatioF();
    int physicalW = qRound(m_mpvWidget->width() * dpr);
    int physicalH = qRound(m_mpvWidget->height() * dpr);
    ...
    bool isUpscaling = (physicalW > vw) || (physicalH > vh);
    QString currentPreset = m_settings.value("defaultShaderPreset", "Off").toString();
    bool shadersOn = (currentPreset != "Off" && !currentPreset.isEmpty());

    if (isUpscaling && shadersOn) {
        QString upscaledName = formatResolution(physicalW, physicalH);
...
{
    animateWidgetOpacity(m_topBar, m_topBarOpacityEffect, 1.0, 250);
    animateWidgetOpacity(m_bottomBar, m_bottomBarOpacityEffect, 1.0, 250);
...
void MainWindow::hideHud()
{
    ...
    animateWidgetOpacity(m_topBar, m_topBarOpacityEffect, 0.0, 250);
    animateWidgetOpacity(m_bottomBar, m_bottomBarOpacityEffect, 0.0, 250);
    animateWidgetOpacity(m_brightnessBar, m_brightnessBarOpacityEffect, 0.0, 250);
    animateWidgetOpacity(m_volumeBar, m_volumeBarOpacityEffect, 0.0, 250);
...
void MainWindow::animateWidgetOpacity(QWidget *widget, QGraphicsOpacityEffect *effect, qreal targetOpacity, int durationMs)
{
    if (!widget || !effect) return;

    QPropertyAnimation *anim = widget->findChild<QPropertyAnimation*>("opacityAnim");
    if (!anim) {
        anim = new QPropertyAnimation(effect, "opacity", widget);
        anim->setObjectName("opacityAnim");
    } else {
        anim->stop();
    }
    ...
    QObject::disconnect(anim, &QPropertyAnimation::finished, nullptr, nullptr);
    if (targetOpacity == 0.0) {
