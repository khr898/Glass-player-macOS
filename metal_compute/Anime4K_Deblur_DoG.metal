...
static float4 hook_pass2(float2 mtlPos, sampler textureSampler, texture2d<float, access::sample> HOOKED, texture2d<float, access::sample> MMKERNEL, texture2d<float, access::sample> MAIN) {
...
kernel void Anime4K_Deblur_DoG_pass2_Anime4K_v3_2_Deblur_DoG_Kernel_Y(
    ...
    sampler textureSampler [[sampler(0)]]) {
    float2 mtlPos = float2(gid) / (float2(output.get_width(), output.get_height()) - float2(1, 1));
    output.write(hook_pass2(mtlPos, textureSampler, HOOKED, MMKERNEL, MAIN), gid);
}
...
static float4 hook_pass3(float2 mtlPos, sampler textureSampler, texture2d<float, access::sample> HOOKED, texture2d<float, access::sample> LINELUMA, texture2d<float, access::sample> MMKERNEL, texture2d<float, access::sample> MAIN) {
...
kernel void Anime4K_Deblur_DoG_pass3_Anime4K_v3_2_Deblur_DoG_Apply(
    ...
    sampler textureSampler [[sampler(0)]]) {
    float2 mtlPos = float2(gid) / (float2(output.get_width(), output.get_height()) - float2(1, 1));
    output.write(hook_pass3(mtlPos, textureSampler, HOOKED, LINELUMA, MMKERNEL, MAIN), gid);
}
