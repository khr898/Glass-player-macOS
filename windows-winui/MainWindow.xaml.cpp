...
namespace winrt::GlassPlayer::implementation
{
    MainWindow::MainWindow()
    {
        ...
        // Register MainWindow in application resources for IPC redirection lookup
        Application::Current().Resources().Insert(box_value(L"MainWindow"), *this);
    }
    ...
    void MainWindow::SetupWindow()
    {
        ...
        // Hook keyboard and pointer movement events
        ...
        // Apply frosted glass to HUD panels
        ApplyFrostedGlass(TopBar(), 30.0f);
        ApplyFrostedGlass(BottomBar(), 30.0f);
        ApplyFrostedGlass(BrightnessBar(), 30.0f);
        ApplyFrostedGlass(VolumeBar(), 30.0f);
    }
    ...
    void MainWindow::SetupCallbacks()
    {
        ...
        m_renderHost->setOnFileLoaded([this]() {
            DispatcherQueue().TryEnqueue([this]() {
                // Read track options, update badge
                std::string widthStr = m_renderHost->getPropertyString("width");
                std::string heightStr = m_renderHost->getPropertyString("height");
                if (!widthStr.empty() && !heightStr.empty()) {
                    ResolutionText().Text(to_wide(widthStr + "x" + heightStr));
    ...
    void MainWindow::ApplyFrostedGlass(FrameworkElement const& host, float blurRadius)
    {
        try {
            ...
            auto visual = ElementCompositionPreview::GetElementVisual(host);
            auto comp = visual.Compositor();

            auto backdrop = comp.CreateBackdropBrush();

            GaussianBlurEffect blur;
            blur.Name(L"Blur");
            blur.BlurAmount(blurRadius);
            blur.Source(CompositionEffectSourceParameter{ L"Backdrop" });

            auto factory = comp.CreateEffectFactory(blur);
            ...
            // Fallback: HUD panels fall back to default theme-defined brushes
        }
    }
}
